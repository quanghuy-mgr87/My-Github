...
namespace SalesManagement.Model.ContentsManagement
{
    ...
    class StaffContents
    {
        ...
        // データ取得（for ComboBox）
        // out コンボボックス用データ（配列）
        public IEnumerable<Staff> GetStaffs_Entity()
        {
        ...
        // out コンボボックス用データ（配列）
        public IEnumerable<Staff> GetStaffsByShop_Entity(int shopId)
        {
            using (var db = new SalesDbContext())
            {
                List<Staff> staff = db.Staffs.Where(m => m.ShopsId == shopId).ToList();
        ...
        // in  : staffId
        // out : staffデータ
        public Staff GetStaff_Entity(int staffId)
        {
            try
            {
                using (var db = new SalesDbContext()) return db.Staffs.Single(m => m.StaffId == staffId);
            }
            catch (Exception ex)
            {
        ...
        {
            using (var db = new SalesDbContext()) if (db.Staffs.Where(m => m.UserId == userId).ToList().Count() != 0) return false;
            return true;
        }
        ...
        // out  : true = パスワードOK
        //      : staff = 従業員情報
        public bool CheckPasswordHash_Entity(string userId, string password, out Staff staff)
        {
            // Saltの読込
            ...
            using (var db = new SalesDbContext())
            {
                ...
                {
                    // ユーザーIDチェック
                    l_staff = db.Staffs.Single(m => m.UserId == userId);
                }
                catch
                {
                    return false;
        ...
        // 表示データ取得
        // out 表示データ（配列）
        public IEnumerable<DispStaff> GetDispStaffs_Entity()
        {
            using (var db = new SalesDbContext())
            {
                List<DispStaff> dispStaffs = new List<DispStaff>();
                foreach (Staff staff in db.Staffs)
                {
                    string shop;
                    try
                    {
                        shop = (staff.ShopsId != -1) ? db.Shops.Single(m => m.ShopCode == staff.ShopsId).ShopName : string.Empty;

                        // 無効表示
                        if (db.Shops.Single(m => m.ShopCode == staff.ShopsId).Status != 0) shop = "[" + shop + "]";
                    }
                    catch
                    {
                        shop = string.Empty;
                    ...
                    try
                    {
                        division = (staff.DivisionsId != -1) ? db.Staffs.Single(m => m.StaffCode == staff.DivisionsId).StaffName : string.Empty;

                        // 無効表示
                        if (db.Divisions.Single(m => m.DivisionCode == staff.DivisionsId).Status != 0) division = "[" + division + "]";
                    }
                    catch
                    {
                    ...
                    dispStaffs.Add(new DispStaff()
                    {
                        ...
                        AddressKana = staff.AddressKana,
                        Phone = staff.Phone,
                        SmartPhone = staff.SmartPhone,
                        Mail = staff.Mail,
                        ...
                        UserId = staff.UserId,
                        // Password = (staff.Password != null) ? _cg.DecryptStringFromBytes_Aes(staff.Password, keys.Key, keys.Iv) : string.Empty,
                        Password = "*****",
                        Comments = staff.Comments,
                        Status = StaticCommon.ConvertToString(1, staff.Status),
                        Timestamp = staff.Timestamp
                    });
                }

                // ソータブルリスト作成
                SortableBindingList<DispStaff> sortableDispStaff = new SortableBindingList<DispStaff>(dispStaffs);
        ...
        // in   startRec : 配列抜出の始点
        //      endRec   : 配列抜出の終点
        public IEnumerable<DispStaff> GetDispStaffs_Entity(int startRec, int endRec)
        {
            using (var db = new SalesDbContext())
            {
                List<DispStaff> dispStaffs = new List<DispStaff>();
        ...
        // データ追加
        // in   : Staffデータ
        public void PostStaff_Entity(Staff regStaff)
        {
            using (var db = new SalesDbContext())
            {
                db.Staffs.Add(regStaff);
        ...
        // データ更新
        // in   : Staffデータ
        public void PutStaff_Entity(Staff regStaff)
        {
            using (var db = new SalesDbContext())
            {
                Staff staff;
                try
                {
                    staff = db.Staffs.Single(x => x.StaffId == regStaff.StaffId);
                }
                catch (Exception ex)
                {
                    throw new Exception(Messages.errorNotFoundStaff, ex);
                ...
                staff.StaffCode = regStaff.StaffCode;
                staff.StaffName = regStaff.StaffName;
                staff.StaffKana = regStaff.StaffKana;
                staff.PostCode = regStaff.PostCode;
                staff.Address = regStaff.Address;
                staff.AddressKana = regStaff.AddressKana;
                staff.Phone = regStaff.Phone;
                staff.Mail = regStaff.Mail;
                staff.BirthDay = regStaff.BirthDay;
                staff.HireDate = regStaff.HireDate;
                staff.ShopsId = regStaff.ShopsId;
                staff.Position = regStaff.Position;
                staff.DivisionsId = regStaff.DivisionsId;
        ...
        public void DeleteStaff_Entity(int StaffId)
        {
            using (var db = new SalesDbContext())
            {
                ...
                try
                {
                    staff = db.Staffs.Single(x => x.StaffId == StaffId);
                }
                catch (Exception ex)
                {
                    throw new Exception(Messages.errorNotFoundStaff, ex);
        ...
        // in       regStaff : ログ対象データ
        // out      string  : ログ文字列
        private string StaffLogData(Staff regStaff)
        {
            string shop;
            string division;
            using (var db = new SalesDbContext())
            {
                try
                {
                    shop = (regStaff.ShopsId != -1) ? db.Shops.Single(m => m.ShopCode == regStaff.ShopsId).ShopName : string.Empty;

                    // 無効表示
                    if (db.Shops.Single(m => m.ShopCode == regStaff.ShopsId).Status != 0) shop = "[" + shop + "]";
                }
                catch
                {
                    shop = string.Empty;
                ...
                try
                {
                    division = (regStaff.DivisionsId != -1) ? db.Staffs.Single(m => m.StaffCode == regStaff.DivisionsId).StaffName : string.Empty;

                    // 無効表示
                    if (db.Divisions.Single(m => m.DivisionCode == regStaff.DivisionsId).Status != 0) division = "[" + division + "]";
                }
                catch
                {
                    division = string.Empty;
            ...
            if (regStaff.HireDate != null) birthDay = regStaff.HireDate.Value.ToShortDateString();

            return regStaff.StaffId.ToString() + ", " +
            regStaff.StaffCode.ToString() + ", " +
            regStaff.StaffName + ", " +
            regStaff.StaffKana + ", " +
            regStaff.PostCode + ", " +
            regStaff.Address + ", " +
            regStaff.AddressKana + ", " +
            regStaff.Phone + ", " +
            regStaff.Mail + ", " +
            birthDay + ", " +
            hireDate + ", " +
            ...
            regStaff.UserId + ", " +
            regStaff.Comments + ", " +
            StaticCommon.ConvertToString(1, regStaff.Status);
        }
    }
}
