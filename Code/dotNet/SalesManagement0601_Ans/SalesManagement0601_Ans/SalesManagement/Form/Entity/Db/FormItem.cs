...
namespace SalesManagement
{
    ...
    public partial class FormItem : Form
    {
        ...
        private bool GetValidDataAtRegistration()
        {
            ...
            if (textBoxItemCD.Text.Trim() == string.Empty) textBoxItemCD.Text = _cc.GetCodeIncrement(Constants.itemCode, out _timeStamp).ToString();
            ...
            // コード重複チェック
            if (!_cc.CheckCodeDuplication(Constants.numItem, textBoxItemCD.Text.Trim(), out errorMessage))
            {
                textBoxItemCD.Focus();
                // labelMessage.Text = errorMessage;
                MessageBox.Show(errorMessage);
        ...
        private M_Item GenerateDataAtRegistration()
        {
            return new M_Item
            {
                ...
                CategoryCD = comboBoxCategorys.SelectedValue.ToString(),
        ...
        private void RegisterItemInformation(M_Item regItem)
        {
            ...
            {
                // コードカウンターインクリメント
                _cc.PutCodeById(Constants.itemCode, _cc.GetCodeIncrement(Constants.itemCode, out _timeStamp), _timeStamp);
            }
        ...
        // JanCD
        private void TextBoxJanCD_Leave(object sender, EventArgs e)
        {
            if (textBoxJanCD.Text == string.Empty) return;

            // 半角英字チェック
            if (!_ic.HalfAlphabetNumericCheck(textBoxJanCD.Text, out string errorMessage)) textBoxJanCD.Focus();
            // labelMessage.Text = errorMessage;
            if (errorMessage != string.Empty) MessageBox.Show(errorMessage);
        }
        ...
        private void TextBoxItemKana_Leave(object sender, EventArgs e)
        {
            ...
            // 半角カナチェック
            if (!_ic.HalfWidthKatakanaCheck(textBoxItemKana.Text, out string errorMessage)) textBoxItemKana.Focus();
            // labelMessage.Text = errorMessage;
            if (errorMessage != string.Empty) MessageBox.Show(errorMessage);
        }
        ...
        private bool InputCheck()
        {
            ...
            // JanCD
            if (textBoxJanCD.Text != string.Empty && !_ic.HalfCharCheck(textBoxJanCD.Text, out errorMessage))
            {
                textBoxJanCD.Focus();
                // if (textBoxJanCD.Text == string.Empty) labelMessage.Text = _cm.GetMessage(10);
            ...
            // 商品半角カナ名
            if (textBoxItemName.Text == string.Empty || !_ic.HalfWidthKatakanaCheck(textBoxItemKana.Text, out errorMessage))
            {
        ...
        private void DataCopy()
        {
            ...
            textBoxJanCD.Text = item.JanCD;
            textBoxListPrice.Text = String.Format("{0:#,0}", item.ListPrice);
            textBoxSellingPrice.Text = String.Format("{0:#,0}", item.SellingPrice);
            comboBoxMakers.SelectedValue = item.MakerID.ToString();
            comboBoxCategorys.SelectedValue = item.CategoryCD;
        ...
        private void InputInitialSetting()
        {
            ...
            textBoxItemKana.ImeMode = ImeMode.KatakanaHalf;
        ...
        private void ComboBoxSetting()
        {
            ...
            comboBoxCategorys.ValueMember = "CategoryCD";
        }
